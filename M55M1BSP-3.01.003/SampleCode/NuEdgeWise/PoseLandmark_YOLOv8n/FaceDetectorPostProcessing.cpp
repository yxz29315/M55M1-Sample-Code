...
namespace arm {
namespace app {
    FaceDetectorPostProcess::FaceDetectorPostProcess(
...
{
    this->m_net = face_detection::Network{
        ...
        .branches =
            {face_detection::Branch{.resolution  = postProcessParams.inputImgCols / 32,
                                      .numBox      = 3,
             ...
             face_detection::Branch{.resolution  = postProcessParams.inputImgCols / 16,
                                      .numBox      = 3,
...
bool FaceDetectorPostProcess::DoPostProcess()
{
	return false;
}

bool FaceDetectorPostProcess::RunPostProcess(std::vector<face_detection::DetectionResult>& results)
{
    int originalImageWidth  = m_postProcessParams.originalImageCols;
    int originalImageHeight = m_postProcessParams.originalImageRows;
    ...
    image::CalculateNMS(detections, this->m_net.numClasses, this->m_postProcessParams.nms);
    ...
    for (auto& it: detections) {
        ...
        if (xMin < 0) xMin = 0;
        if (yMin < 0) yMin = 0;
        if (xMax > originalImageWidth) xMax = originalImageWidth;
        if (yMax > originalImageHeight) yMax = originalImageHeight;
        float boxX = xMin;
        float boxY = yMin;
...
void FaceDetectorPostProcess::InsertTopNDetections(std::forward_list<image::Detection>& detections, image::Detection& det)
{
...
void FaceDetectorPostProcess::GetNetworkBoxes(
        face_detection::Network& net,
        int imageWidth,
        int imageHeight,
        float threshold,
        std::forward_list<image::Detection>& detections)
