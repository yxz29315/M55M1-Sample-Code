...
)
{
    ...
    f_write(psWavFileObject, abySignature1, 4, &ReturnSize);
    ...
    f_write(psWavFileObject, &u32Len, 4, &ReturnSize);
    ...
    f_write(psWavFileObject, abySignature2, 4, &ReturnSize);
...
)
{
    ...
    f_write(psWavFileObject, &u32WavFormatDataLen, 4, &ReturnSize);

    if (4 != ReturnSize)
        return FALSE;

    f_write(psWavFileObject, pWavFormat, u32WavFormatDataLen, &ReturnSize);
    ...
    f_write(psWavFileObject, &u32ChunkLen, 4, &ReturnSize);
...
{
	FIL *psWavFileObject = (FIL *) psInfo->i32FileHandle;

    return WavFileUtil_Write_SetFormatInternal(
               psWavFileObject, psInfo, psWavFormatEx, WAV_FORMAT_EX_LEN);
...
)
{
    ...
    return WavFileUtil_Write_SetFormatInternal(
               psWavFileObject, psInfo, psIMAWavFormat, IMA_WAV_FORMAT_LEN);
...
)
{
	...
	FIL     *psWavFileObject = malloc(sizeof(FIL));
	
	if(psWavFileObject == NULL)
		return FALSE;
	
	memset(psWavFileObject, 0x0, sizeof(FIL));
	
    res = f_open(psWavFileObject, (const TCHAR *)pszOutFileName, FA_CREATE_ALWAYS | FA_WRITE);      //USBH:0 , SD0: 1
    ...
    psInfo->i32FileHandle = (INT32)psWavFileObject;
    ...
    if (!WavFileUtil_Write_WriteHeader(psWavFileObject, psInfo))
    {
        f_close(psWavFileObject);
...
)
{
    ...
    f_write(psWavFileObject, pbyData, u32DataSize, &ReturnSize);
...
)
{
    ...
    if (0 != f_close(psWavFileObject))
        return FALSE;

    f_stat(pszOutFileName, &finfo);

    UINT32 u32Pos = finfo.fsize;

    FRESULT res;

    res = f_open(psWavFileObject, (const TCHAR *)pszOutFileName, FA_OPEN_EXISTING | FA_WRITE);      //USBH:0 , SD0: 1
            ...
            (u32Pos > psInfo->u32DataChunkLenFilePos))
    {
        ...
        if (0 != f_lseek(psWavFileObject, psInfo->u32RiffDataLenFilePos))
            return FALSE;
        ...
        if (0 != f_lseek(psWavFileObject, psInfo->u32DataChunkLenFilePos))
            return FALSE;
