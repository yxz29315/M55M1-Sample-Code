...
static void Configure_SPI_Pins(void)
{
    ...
    // Enable Schmitt trigger on GPIO pins for high-speed signals to improve input signal integrity
    PA->SMTEN |= GPIO_SMTEN_SMTEN10_Msk;

    /* Enable SPI2 I/O high slew rate */
    // Because this is a high-speed signal, configure the GPIO for high slew rate to improve output signal quality
...
{
    uint32_t u32WaiteCycles = get_ticks();

    u32WaiteCycles +=  u32MilliSec * 1;

    while (get_ticks() <= u32WaiteCycles)
    {
        __NOP();
    }
}
