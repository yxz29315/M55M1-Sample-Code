...
{
#if defined(__ZEPHYR__)
    IRQ_DIRECT_CONNECT(ETHOS_U_IRQN, 0, NPU_Isr, 0);
    irq_enable(ETHOS_U_IRQN);

    debug("EthosU IRQ#: %u, Handler: 0x%p\n",
          ethosu_irqnum, NPU_Isr);
...
struct k_mutex ethosu_mutex;
struct k_mutex ethosu_semaphore;

void *ethosu_mutex_create(void)
{
    k_mutex_init(&ethosu_mutex);
    return &ethosu_mutex;
}

int ethosu_mutex_lock(void *mutex)
{
    struct k_mutex *handle = (struct k_mutex*)mutex;
    k_mutex_lock(handle, K_FOREVER);
    return 0;
}
