...
static void nu_pdma_init(void)
{
    ...
    for (i = (PDMA_START + 1); i < PDMA_CNT; i++)
    {
        ...
        if(psPDMA == PDMA0)
        {
            IRQ_DIRECT_CONNECT(PDMA0_IRQn, 0, PDMA0_Isr, 0);
        }
        else if(psPDMA == PDMA1)
        {
            IRQ_DIRECT_CONNECT(PDMA1_IRQn, 0, PDMA1_Isr, 0);
        }

        irq_enable(nu_pdma_arr[i].eIRQn);
#else
...
static void nu_pdma_memfun_actor_init(void)
{
    ...
    for (i = 0; i < NU_PDMA_MEMFUN_ACTOR_MAX; i++)
    {
        ...
        {
#if defined(OS_ZEPHYR)
            k_sem_init(&nu_pdma_memfun_actor_arr[i].m_psSemMemFun, 0, 1);
#else
